package com.gaa.appdev.subscription.sample.billing
...
class PurchaseManager(private val activity: Activity, private val callback: Callback) : PurchasesUpdatedListener {
     ...
     */
    fun queryProductDetailAsync(productIdList: List<String>, @PurchaseClient.ProductType productType: String?, listener: ProductDetailsListener?) {
        executeServiceRequest {
     ...
     * @param types 상품타입
     */
    fun queryPurchasesAsync(@PurchaseClient.ProductType vararg types: String) {
    ...
    private fun handleError(iapResult: IapResult) {
        when (iapResult.responseCode) {
            ...
            PurchaseClient.ResponseCode.RESULT_NEED_UPDATE -> {
                Log.w(TAG, "handleError() RESULT_NEED_UPDATE")
