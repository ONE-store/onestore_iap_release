package com.onestore.sample.subscription.billing
 ...
 */
object AppSecurity {
    private val TAG = AppSecurity::class.java.simpleName
    ...
    /**
     * 앱을 등록 시에 서버에 private, public key로 한 쌍이 생성되며
     * 앱에서는 퍼블릭키를 통해서 해당 상품의 위변조 여부를 조회하는 전자 서명을 조회를 위한 용도로 사용한다.
     * public key는 안전하게 저장하는 것이 중요하여 백엔드 서버에 저장하는 것이 좋습니다.
     */
    const val PUBLIC_KEY = "input your license key"

    fun verifyPurchase(publicKey: String, signedData: String, signature: String): Boolean {
        Log.d(TAG, "\n========== Security verifyPurchase ==========")
        Log.d(TAG, "BASE64 PUBLICKEY :: $publicKey")
